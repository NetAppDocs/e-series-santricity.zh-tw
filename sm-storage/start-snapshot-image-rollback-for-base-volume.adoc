...
====
.步驟
. 從左側面板中，選擇功能表：Storage[Snapshots]。
...
.. 使用微調方塊來調整容量百分比。
+
如果包含所選儲存物件的儲存池或磁碟區組上沒有可用容量，且儲存陣列具有 Unassigned Capacity，則可以 Add capacity。您可以建立一個新的儲存池或磁碟區組，然後使用該儲存池或磁碟區組上的新可用容量重試此操作。
...
.完成後
選擇功能表：Storage[Operations in Progress] 以檢視復原作業的進度。
