...
您可以允許 System Manager 在偵測到儲存陣列中未指派的容量時自動建立儲存池。或者，當自動建立無法確定最佳配置時，您可以從選單：Storage[Pools and volume groups] 手動建立儲存池。
...
您可以從選單建立磁碟區群組：Storage[Pools and volume groups]。
