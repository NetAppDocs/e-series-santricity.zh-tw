...
summary: 您可以編輯 SSD Cache 的設定。目前 SSD Cache 的設定可透過「檢視設定」選項存取。 
---
...
[role="lead"]
您可以編輯 SSD Cache 的設定。目前 SSD Cache 的設定可透過「檢視設定」選項存取。

.步驟
. 從左側面板中選擇功能表：Storage[Pools and volume groups]。
. 選擇要編輯的 SSD Cache，然後按一下 menu:ellipsis icon[Edit Settings]。
+
「編輯 SSD 快取設定」視窗隨即出現。
